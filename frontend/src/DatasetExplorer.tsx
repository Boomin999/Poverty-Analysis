  ...
  return (
    <Layout>
      <div className="space-y-8">
        <section>
          <Label className="mb-2 block">Data Repository</Label>
          <Headline level={1} className="mb-4">Dataset Explorer</Headline>
          <p className="text-on-surface/60 max-w-2xl">
            Browse and preview the datasets behind the dashboard and chat experience. Select a dataset to inspect its metadata and a small data sample.
          </p>
        </section>
        ...
        <div className="space-y-6">
          ...
          <Card className="min-h-[500px] flex flex-col p-0 overflow-hidden">
            {selectedDataset && selectedDataset ? (
              <>
                <div className="p-6 border-b border-outline-variant bg-surface-container-low flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-1">
                      ...
                      <span className="text-[10px] text-on-surface/20">•</span>
                      <span className="text-[10px] font-bold text-on-surface/40 uppercase">{selectedDataset.source}</span>
                    </div>
                    <p className="text-sm text-on-surface/60 mt-3">{selectedDataset.description}</p>
                  </div>
                  <Button variant="secondary" className="px-4 py-2 w-fit">
                    <Download size={16} className="mr-2 inline" /> Preview Loaded
                  </Button>
                </div>
                ...
                <div className="flex-1 overflow-auto">
                  {isLoading ? (
                    ...
                    <div className="h-full flex items-center justify-center text-sm text-on-surface/50">Select a dataset to load its preview.</div>
                  )}
                </div>
              </>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-12">
                ...
                <Headline level={3} className="text-on-surface/40">Select a dataset to preview</Headline>
                <p className="text-sm text-on-surface/30 mt-2 max-w-xs">
                  Choose a dataset from the dropdown above to inspect its metadata and sample records from the new API.
                </p>
              </div>
            )}
          </Card>
